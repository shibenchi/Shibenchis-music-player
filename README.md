...
I built this because I didn't want to keep paying for Spotify or YouTube Premium, and every other free music player out there either got discontinued or had its good features ripped out. So — this. Search, queue, playlists, downloads, all in one place, no subscription, no ads.
...
The actual big change here: this used to run through your browser — double-click a batch file, wait for it to compile, open `localhost:3000`, hope nothing was already using that port. That was pretty archaic and honestly kind of a pain, sorry about that. It's a real native desktop app now. Install it with an actual **MSI installer** like any other windows program, no terminal, no browser tab, no batch files to babysit. It's also just faster this way.
...
- everything from before — search, queue, playlists, downloads, EQ, theme color, background visualizers — still here and working
- a bunch of bug fixes I won't bore you with in the readme
- **social features (friends, DMs, collab servers) are out for now** — they depended on a shared backend that isn't running anymore. the code's still in there, it's just not reachable, so I turned it off instead of shipping something broken
...
2. run it
3. done — it shows up in your start menu like anything else you've installed
...
bugs, questions, whatever — reach out, I don't bite.
